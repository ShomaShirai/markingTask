...
# 追加したい機能
- assetsディレクトリの各グループの画像を使って表示する機能を`services/asset_service.py`で提供。
	- UI（`interface/main_window.py`）はサービスを呼び出して、初期表示および「次へ行く」でランダムにグループ切り替え。
	- 期待するファイル名（例: `woman.png`, `compositionMip.png`, `vein_white.png`）が揃ったディレクトリを1グループとして検出。
	- グループが無い場合は従来の`assets`直下の3ファイルにフォールバック。